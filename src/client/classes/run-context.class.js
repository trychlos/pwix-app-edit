...
export class RunContext extends mix( CoreApp.RunContext ).with( IAppEditable ){
    ...
    constructor( o ){
        ...
        this.title( AppEdit.configure().appName );
        ...
        Tracker.autorun(() => {
            ...
            if( id !== self.#user.get()){
                _verbose( AppEdit.C.Verbose.PAGE, 'pwix:app-edit setting \''+id+'\' as current user' );
                self.#user.set( id );
            }
        });
        */
        ...
        Tracker.autorun(() => {
            if( Roles.ready()){
                ...
                if( !_.isEqual( roles, self.#roles.get())){
                    _verbose( AppEdit.C.Verbose.PAGE, 'pwix:app-edit setting current roles', roles );
